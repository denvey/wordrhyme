...
- [ ] 验证 `storage:provider` 能力声明
- [ ] 自动注入 pluginId 到 metadata
- **验证**: 插件可注册存储提供者
...
- [ ] 创建 `plugins/storage-s3/` 目录结构
- [ ] 实现 `S3StorageProvider` 类
- [ ] 定义配置 JSON Schema
- **验证**: S3 上传/下载/签名 URL 正确

### 10.2 Create plugin-storage-oss
- [ ] 创建 `plugins/storage-oss/` 目录结构
- [ ] 实现 `OSSStorageProvider` 类
- [ ] 定义配置 JSON Schema
- **验证**: OSS 上传/下载/签名 URL 正确

### 10.3 Create plugin-storage-r2
- [ ] 创建 `plugins/storage-r2/` 目录结构
- [ ] 实现 `R2StorageProvider` 类（基于 S3 兼容 API）
- [ ] 定义配置 JSON Schema
- **验证**: R2 上传/下载/签名 URL 正确
