 ...
 */
export const RETENTION_POLICIES: Array<{
...
export const notifications = pgTable(
  'notifications',
  {
    ...
    aggregationStrategy: text('aggregation_strategy').notNull().default('none').$type<AggregationStrategy>(),
  ...
  (table) => ({
    ...
    templateIdx: index('idx_notifications_template').on(table.templateKey),
    ...
    sourceIdx: index('idx_notifications_source').on(table.source),
    pinnedIdx: index('idx_notifications_pinned').on(table.pinned),
    cleanupIdx: index('idx_notifications_cleanup').on(table.category, table.read, table.createdAt),
  })
);
