...
export const cacheRouter = router({
  ...
  scanKeys: protectedProcedure
    .input(
      z.object({
        namespace: z.string().min(1).describe('Namespace pattern (e.g., "org:123:*")'),
  ...
  previewInvalidation: protectedProcedure
    .input(
      z.object({
        namespace: z.string().min(1).describe('Namespace (e.g., "org:123")'),
    ...
    .query(async ({ input }) => {
      try {
        ...
        if (parts[0] === 'org' && parts[1]) {
          ...
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Invalid namespace format. Expected "org:{id}" or "plugin:{id}"',
          });
        }
   ...
   *
   * Scans for "org:*" keys and extracts unique organization IDs.
   */
  listOrganizations: protectedProcedure.query(async () => {
    ...
    try {
      ...
      // Scan all organization keys
      do {
        const result = await admin.scan('org:*', cursor, 100);

        // Extract organization IDs from keys
      ...
      throw new TRPCError({
        code: 'INTERNAL_SERVER_ERROR',
        message: `Failed to list organizations: ${error}`,
      });
    }
  }),
