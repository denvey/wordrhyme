...
### 5.5 SSR 集成约束（与 i18n 一致）

**为什么不能用全局函数**：

| 问题 | 全局 p() | Hook 模式 |
|------|-----------|-----------|
| SSR 请求隔离 | ❌ 全局状态在多请求间共享 | ✅ 每请求独立 Context |
| Hydration 一致性 | ❌ 可能服务端/客户端不一致 | ✅ 数据通过 Props 传递 |
| React 18 并发 | ❌ 不兼容 Concurrent Mode | ✅ 完全兼容 |
...
**服务端代码（Server Component）**：

```tsx
// src/app/[locale]/layout.tsx
export default async function Layout({ children }) {
  // 服务端预取货币数据
  const currencyData = await fetchCurrencyData(organizationId);
...
export function CurrencyProvider({
  ...
  const value = useMemo(() => ({
    ...
    version,
    isLoading: false,
  }), [p, userCurrency, baseCurrency, rates, currencies, version]);
...
**约束**：
- ✅ 服务端预取数据，通过 Props 传给 CurrencyProvider
- ✅ p 函数通过 useCurrency() Hook 获取，不是全局函数
- ✅ 货币切换通过 Context 更新，触发组件重新渲染
- ❌ 禁止将 p 挂载到 window 全局对象
- ❌ 禁止在 Server Component 中直接使用 useCurrency()
