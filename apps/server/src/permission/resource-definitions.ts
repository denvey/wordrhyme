 ...
 * 基础资源定义类型
 */
interface BaseResourceDefinition<S extends string = string, A extends string = string> {
    /** CASL Subject 标识 */
    subject: S;
    /** 资源分类（用于 UI 分组和排序） */
...
export const RESOURCE_DEFINITIONS = {
    ...
    //     icon: "KeyRound",
    //     menuPath: "/settings/general?tab=oauth",
    //     actions: ["read", "update"] as const,
 ...
 * 使用类型断言保持精确类型
 */
export const Subjects = Object.fromEntries(Object.entries(RESOURCE_DEFINITIONS).map(([k, v]) => [k, v.subject])) as {
    [K in ResourceKey]: (typeof RESOURCE_DEFINITIONS)[K]["subject"];
};
