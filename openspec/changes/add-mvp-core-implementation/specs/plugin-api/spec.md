...
A separate npm package `@nebula/plugin-api` SHALL be created. This package MUST export TypeScript types and runtime helpers for plugin authors. Plugins SHALL only import from `@nebula/plugin-api`, never from `@nebula/core`.

#### Scenario: Plugin imports API package
- **WHEN** a plugin imports `import { definePlugin } from '@nebula/plugin-api'`
...
- **THEN** TypeScript validates required fields: `pluginId`, `version`, `vendor`, `type`, `runtime`, `engines.nebula`
