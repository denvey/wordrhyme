...
# Project Instructions (WordRhyme)

本项目的核心架构、数据库、权限和前后端 CRUD 规范已拆分存放在 `.agents/skills/` 目录下。作为 AI Assistant，在针对特定领域进行开发前，**务必先读取**对应的规范文件：

- 架构总览与底层限制（必看）：`.agents/skills/wordrhyme-architecture/SKILL.md`
- 数据库 Drizzle/Zod 规范：`.agents/skills/wordrhyme-db/SKILL.md`
- 后端 Service/Repo 分层：`.agents/skills/wordrhyme-backend-patterns/SKILL.md`
- 菜单展示规则：`.agents/skills/wordrhyme-menus/SKILL.md`
- 前后端 CRUD 最佳实践：`.agents/skills/wordrhyme-crud/SKILL.md`
